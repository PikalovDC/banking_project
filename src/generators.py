...
def card_number_generator(start: int, end: int) -> Generator[str, None, None]:
    ...
    if start < min_number:
        raise TypeError(f"Начальное значение не может быть меньше {min_number}")

    if end > max_number:
        raise TypeError(f"Конечное значение не может быть больше {max_number}")

    if start > end:
        raise TypeError("Начальное значение не может быть больше конечного")
